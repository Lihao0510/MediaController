...
public class Encoder implements SinGenerator.Listener, SinGenerator.Callback {
    ...
    // index 0, 1, 2, 3, 4, 5, 6
    // sampling point Count 0-1900, 1-1950, 2-2000 ......
    private final static int[] CODE_FREQUENCY = {1900, 1950, 2000, 2050, 2100, 2150, 2200, 2250, 2300, 2350, 2400, 2450, 2500, 2550, 2600, 2650};
    private int mState;
    ...
    // 按频率编码声音，在最后加上指定muteInterval时间的空白时长
    public void encode(List<Integer> codes, int duration, int muteInterval) {
        if (STATE_STOPED == mState) {
            ...
            LogHelper.d(TAG, "codes:" + codes.toString());
            mSinGenerator.start();
            for (int index : codes) {
